...
class RandomPlayer:
    ...
    def get_move(self, board, cube):
        r = rand.randint(0, 3)
        return MOVES[r]

class GeneticPlayer:

    def __init__(self, pop_size, num_generations, num_trails,
                 window_size, hidden_size, board_size, mutation_chance=0.1, mutation_size=0.1):
        self.pop_size = pop_size
        self.num_generations = num_generations
        self.num_trails = num_trails  # trails in each brain
        self.window_size = window_size  # board size for brain size x size for head surround
        self.hidden_size = hidden_size  # brain size
        self.board_size = board_size

        self.mutation_chance = mutation_chance  # % of mutation
        ...
        self.pop = [self.generate_brain(self.window_size ** 2, self.hidden_size, len(MOVES))
                    for _ in range(self.pop_size)]

        self.evolve_pop()

    ## Newton Network
    ## brain:                      input vector multiply to layer1 then times layer2 then times output layer
    ## layer1       [matrix]       output layer multiply vector == network
    ## layer2       [matrix]       I*W = i1 w1 + i2 w2 + ... in wn = aw au wv
    ## output layer [matrix]       return largest value for next generation
    ...
    def get_move(self, board, cube):
        input_vector = self.process_board(board, cube[-1][0], cube[-1][1])
        hidden_layer1 = self.current_brain[0]
        hidden_layer2 = self.current_brain[1]
        output_layer = self.current_brain[2]

        # Forward prop
        # input_vector * matrix1  tanh keep the value in range -1 < x < 1
        hidden_result1 = np.array(   # [1] for bias
            [math.tanh(np.dot(input_vector, hidden_layer1[i])) for i in range(hidden_layer1.shape[0])] + [1])
        hidden_result2 = np.array(
            [math.tanh(np.dot(hidden_result1, hidden_layer2[i])) for i in range(hidden_layer2.shape[0])] + [1])
        output_result = np.array(
            [np.dot(hidden_result2, output_layer[i]) for i in range(output_layer.shape[0])])
        
        max_index = np.argmax(output_result)
        return MOVES[max_index]

    def process_board(self, board, x1, y1):
        # position around cube head
        input_vector = [[0 for _ in range(self.window_size)] for _ in range(self.window_size)]

        for i in range(self.window_size):            # index the window
            for j in range(self.window_size):
                ii = x1 + i - self.window_size // 2  # index the board, get half the window size from cube head
                jj = y1 + j - self.window_size // 2

                # if window out of bounds, cube cannot move there
                if ii < 0 or jj < 0 or ii >= self.board_size or jj >= self.board_size:
                    input_vector[i][j] = -1
                elif board[ii][jj] == EMPTY:
                    input_vector[i][j] = 1
                elif board[ii][jj][1] == 'C':
                    if int(board[ii][jj][0]) == 1:
                        input_vector[i][j] = -1
                    else:
                        input_vector[i][j] = 2
                elif board[ii][jj][1] == 'T':
                    if int(board[ii][jj][0]) == 1:
                        input_vector[i][j] = 2
                else:
                    input_vector[i][j] = -1
        if self.display:
            # to see what brain saw
            print(np.array(input_vector))
        input_vector = list(np.array(input_vector).flatten()) + [1]
        return np.array(input_vector)

    ## get top_25 to do mutation
    ...
    def one_generation(self):
        ...
        for i in range(self.pop_size):
            for j in range(self.num_trails):
                ...
                score = len(game.territory[0])
                scores[i] += score

                if outcome == 0:
                    print("Player", i, "made it to the last turn")
                if score > max_score:
                    max_score = score
    ...
    def evolve_pop(self):
        ...
        for brain in self.pop:
            ...
            print("Territory occupied", len(game.territory[0]))